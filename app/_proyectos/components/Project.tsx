import Image from "next/image";
import TechIcosList from "./TechIconsList";

interface ProjectProps {
  title: string;
  description1: string;
  description2: string;
  imgSrc: string;
  techs: string[];
  githubLink: string;
  demo?: string;
}

export default function Project({
  title,
  description1,
  description2,
  imgSrc,
  techs,
  githubLink,
  demo,
}: ProjectProps) {
  return (
    <>
      <div className="mb-0 h-full flex flex-col">
        <div className="relative w-full aspect-video rounded-xl overflow-hidden border border-black/5 group shadow-sm">
          <img src={imgSrc} alt={title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
          {techs && techs.length > 0 && (
            <div className="absolute bottom-3 right-3 h-10 px-1 bg-almost-black/70 backdrop-blur-md rounded-lg flex items-center shadow-md sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
              <TechIcosList techs={techs} />
            </div>
          )}
        </div>
        <h3 className="mt-4 text-xl">{title}</h3>
        <div className="flex-grow flex flex-col mt-2 mb-4">
          <p className="text-light-gray line-clamp-3">{description1}</p>
        </div>
        <div className="flex items-center justify-between mt-auto">
          <a
            className="px-4 py-2 text-almost-black border border-almost-black/30 rounded-lg text-sm hover:bg-almost-black hover:text-white transition-all duration-300 flex items-center gap-2"
            href={githubLink || '#'}
            target={githubLink ? "_blank" : "_self"}
            rel="noopener noreferrer"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" /></svg>
            Ir al repo
          </a>
          {demo && (
            <a
              href={demo}
              target="_blank"
              rel="noopener noreferrer"
              className="px-4 py-2 bg-background-blue/10 text-background-blue border border-background-blue/30 rounded-lg text-sm hover:bg-background-blue hover:text-white transition-all duration-300 flex items-center gap-2"
            >
              Ver en vivo
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>
            </a>
          )}
        </div>
      </div>
    </>
  );
}
